  ...
  describe Adapter do
    ...
    describe "adapter instantiation" do
      ...
      it "should set opts as an empty has unless variable is set" do
        during_process { 
          RosettaQueue::Gateway::AmqpSynchAdapter.should_receive(:new).with({:user => "foo", :password => "bar", :host => "localhost", :port => "9000", :opts => {}})
        }
      end 
      ...
      describe "when setting options" do
        ...
        it "should map adapter_settings to a hash" do
          during_process { 
            RosettaQueue::Gateway::AmqpSynchAdapter.should_receive(:new).with({:user => "foo", :password => "bar", :host => "localhost", :port => "9000", :opts => {:vhost => "baz"}})
          }
        end 
      end 
